...
# Libraries: Import essential libraries (pandas for data manipulation, numpy for numerical operations, plotly.express for interactive plots).
# Display Options: Set display options to handle large dataframes by increasing the number of rows and columns displayed.
...
# Date of Birth to Age: Convert the dob column to a datetime format and calculate age in years.
# Duplicate Check: Identify and count duplicated rows.
# Basic Statistics: Display summary statistics for numerical features.
# Dataset Info: Show data types and non-null counts.
# Missing Values: Check for and count missing values.
...
# Categorical Features: Identify categorical features in the dataset.
# Class Distribution: Calculate and print the percentage of fraud vs. non-fraud transactions.
# Bar Plot: Plot the distribution of fraud classes.
...
# Save Models and Transformers: Use pickle to save the trained model, scalers, and encoders for future use.


# Summary
# This notebook covers the entire data science workflow:

# Data Loading: Import data and initial inspection.
# Data Cleaning: Handle missing values, remove irrelevant columns, and engineer features.
# Encoding: Convert categorical features into numerical formats.
# Scaling: Standardize features.
# Model Training: Train and evaluate various classification models.
# Model Saving: Save the trained models and preprocessing objects for later use.